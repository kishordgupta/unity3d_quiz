...
///////////// NUMERIC FUNCTION WITH RECORD DESTAIL//////////////////////////////////////
	function number_pageing($query,$record_per_page='',$pages='',$detail='',$bottom='',$simple='',$cur_page="",$query_string="")
	{
		...
		{ 
			...
			$this->tbl.="
			<a href=".$cur_page."?start=".($start-$this->record_per_page).$query_string."><< Previous</a> ";		
		}
		else
		{
			$this->tbl.="<< Previous ";
		...
		if($simple!='N')
		{
			...
			for($i=$start_loop;$i<$end_loop;$i++) 
			{
				if($current_page==$i)	
				{
					...
					$this->tbl.="<a class='number current' href='".$cur_page."?start=".($i-1)*$this->record_per_page.$query_string."'>".$i."</a> ";
				}	
				else 
				{ 
					...
					$this->tbl.="<a class='number' href='".$cur_page."?start=".($i-1)*$this->record_per_page.$query_string."'>".$i."</a> ";
		...
		{ 
			...
			$this->tbl.="<a href='".$cur_page."?start=".($start+$this->record_per_page).$query_string."'>Next</a> ";
